...
func (h *Handler) AllShardsBalancers(c *gin.Context) {
	shards, err := h.balancerService.AllShardsBalancers()
	...
	if err != nil {
		...
		c.JSON(404, dto.NewErrorAllShardsBalancersResponse(
			balancer.ErrNotFound.Error(),
		))
		return
	}

	c.JSON(200, dto.NewSuccessAllShardsBalancersResponse(shards))
}
...
func (h *Handler) Create(c *gin.Context) {
	...
	h.policy.CanCreate().
		Allow(func() {
			n, err := h.balancerService.Create(u.ID, req.Address, req.ShardAssignments)
